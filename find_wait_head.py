...
from find_wait_head_lib.ablation import SingleHeadAblationHook, filter_heads, list_all_heads, parse_head_label
...
from find_wait_head_lib.parallel_utils import run_ablation_worker
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of worker processes for parallel ablation. 0 means auto=min(num_gpus, num_selected_heads).",
    )
    parser.add_argument(
        "--baseline_gpu_id",
        type=int,
...
def main() -> None:
    ...
    if can_parallel_heads and args.baseline_gpu_id not in available_gpu_ids:
        raise ValueError(
    ...
    use_parallel_heads = can_parallel_heads and len(selected_heads) > 1
    if use_parallel_heads:
        print(
            "[Info] Parallel ablation enabled: "
            f"gpus={available_gpu_ids}, selected_heads={len(selected_heads)}"
        )
    else:
        print("[Info] Parallel ablation disabled; using single-process ablation loop.")
    ...
    print(f"- Wait logit JSONL:         {wait_logit_path}")
    print(f"- Wait logit ranking CSV:   {wait_logit_rank_path}")
    print(f"- Parallel ablation mode:   {use_parallel_heads}")
    print(f"- Head summary CSV:        {summary_path}")
    print(f"- Run config:              {config_path}")
    print(f"- Ablation position:       {ABLATION_POSITION}")
