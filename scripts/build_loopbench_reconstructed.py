...
LOOPBENCH_MODULE = _load_module(
    "loopbench_reconstructed_local",
    ROOT_DIR / "cot_research" / "loopbench_reconstructed.py",
)
...
def write_readme(path: Path, *, summary: dict) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    text = f"""# LoopBench Reconstructed

This directory contains a paper-faithful reconstruction of LoopBench based on arXiv:{summary['paper_reference']['arxiv_id']}.

- Total examples: `{summary['total_examples']}`
- Task split: `7` subtasks, `100` samples each
- Public prompt fragment source: `Figure 10`
- Decoding setup for the baseline script: `{summary['paper_baseline_decoding']}`
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a paper-faithful reconstructed LoopBench dataset from the public appendix."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=str(ROOT_DIR / "evaluation" / "data" / "loopbench_reconstructed"),
    )
...
def main() -> None:
    ...
    rows = LOOPBENCH_MODULE.build_loopbench_reconstructed_dataset(
        per_task=args.per_task,
        seed=args.seed,
    )
    summary = LOOPBENCH_MODULE.build_loopbench_reconstructed_summary(rows)
    ...
    write_readme(output_dir / "README.md", summary=summary)

    print(
        f"Built loopbench_reconstructed dataset: total_examples={len(rows)} "
